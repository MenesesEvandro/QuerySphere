...
<body>
    ...
    <div class="card shadow-lg border-0 connection-card">
        <div class="card-body p-5">
            ...
            <p class="card-subtitle mb-4 text-center text-muted"><?= lang('App.connectionScreenTitle') ?></p>
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control" id="port" name="port" value="<?= old('port', '1433') ?>"
                    required>
            </div>
            <div class="mb-3">
                <label for="database" class="form-label"><?= lang('App.database') ?></label>
                <input type="text" class="form-control" id="database" name="database" value="<?= old('database') ?>">
            </div>
            <div class="mb-3">
                <label for="user" class="form-label"><?= lang('App.user') ?></label>
            ...
            <div class="d-grid mt-4">
                <button type="submit" class="btn btn-primary btn-lg"><?= lang('App.connect') ?></button>
            </div>
            <?= form_close() ?>
        </div>
        <div class="card-footer text-center py-3 bg-light d-flex justify-content-between align-items-center">
            <div>
                <a href="<?= site_url('check') ?>" class="text-decoration-none text-muted" style="font-size: 0.9em;">
                    <i class="fa fa-check-circle me-1"></i> <?= lang('App.server_compatibility_check') ?>
                </a>
            </div>
            <div>
                <a href="<?= site_url('lang/pt-BR') ?>" class="text-decoration-none me-3">Português (BR)</a>
                <a href="<?= site_url('lang/en-US') ?>" class="text-decoration-none">English (US)</a>
            </div>
        </div>
    </div>
