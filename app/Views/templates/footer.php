    ...
    const schemaEditor = {
        ...
        open: function(mode, nodeData) {
            ...
            if (mode === 'create') {
                ...
                $('#schemaEditorModalLabel').text(`${LANG.design_table}: ${this.tableName}`);
                ...
                ) ?>/${encodeURIComponent(this.dbName)}/${encodeURIComponent(this.schemaName)}/${encodeURIComponent(this.tableName)}`)
                    .done(columns => {
                    ...
                    .done(indexes => {
                        ...
                        if (indexes && indexes.length > 0) {
                            indexes.forEach(index => {
                                const unique = index.is_unique ? '<i class="fa fa-check-circle text-success"></i>' : '';
                                const row = `
                                    <tr>
                            ...
                            container.html('<tr><td colspan="4" class="text-muted">No indexes found for this table.</td></tr>');
                        }
                    });
            }
        ...
        createTable: function() {
            ...
            $.ajax({
                ...
                success: (response) => {
                    notifier.show(LANG.feedback.table_created_successfully.replace('{0}', tableName), 'success');
                    this.modal.hide();
                    $('#object-explorer-tree').jstree(true).refresh();
                },
                error: (xhr) => {
                    const errorMsg = xhr.responseJSON?.messages?.error || LANG.feedback.table_creation_failed;
                    notifier.show(errorMsg, 'error');
                }
            });
        },
        ...
        alterTable: function() {
            ...
            Promise.all(promises)
                ...
                .catch((xhr) => {
                    const errorMsg = xhr.responseJSON?.messages?.error || 'Failed to update table.';
                    notifier.show(errorMsg, 'error');
                });
        }
    };
