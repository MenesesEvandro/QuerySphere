...
class SqlServerModel extends Model
{
    ...
    public function tryConnect(array $credentials): array
    {
        ...
        if ($conn) {
            sqlsrv_close($conn);
            return ['status' => true, 'message' => lang('App.connection_success')];
        } else {
            $errors = sqlsrv_errors();
            $errorMessage = lang('App.connection_failed') . " ";
            if (is_array($errors) && !empty($errors)) {
                 $errorMessage .= lang('App.details') . ": " . $errors[0]['message'];
            } else {
    ...
    public function executeQuery(string $sql, int $page = 1, int $pageSize = 1000): array
    {
        ...
        if ($stmt === false) {
            $errors = sqlsrv_errors();
            return ['status' => 'error', 'message' => lang('App.syntax_error') . ($errors[0]['message'] ?? lang('App.unknown_error'))];
        }
    ...
    public function getExecutionPlan(string $sql): array
    {
        ...
        if (empty($xmlPlan)) {
            return ['status' => 'error', 'message' => lang('App.execution_plan_generation_failed')];
        }
